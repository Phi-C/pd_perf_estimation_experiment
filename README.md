...
$$
\begin{aligned}
\mathrm{inputTPS} &= \frac{\sum \mathrm{ISL}}{T}, \\
\mathrm{outputTPS} &= \frac{\sum \mathrm{OSL}}{T}, \\
\mathrm{totalTPS} &= \frac{\sum\mathrm{ISL}+\sum\mathrm{OSL}}{T}
\end{aligned}
$$

对应 AIPerf 字段：`input_token_throughput` / `output_token_throughput` / `total_token_throughput`。

按请求切阶段（首 token 为界；ITL **不含** TTFT；OSL $=1$ 时 ITL 无定义）：

$$
\mathrm{TTFT}=t_{\mathrm{first}}-t_{\mathrm{send}},\qquad
\mathrm{ITL}=\frac{\mathrm{latency}-\mathrm{TTFT}}{\mathrm{OSL}-1}
$$

$$
\mathrm{prefillTPS_{user}}=\frac{\mathrm{ISL}}{\mathrm{TTFT}},\qquad
\mathrm{outputTPS_{user}}=\frac{1}{\mathrm{ITL}}
$$
...
请求 $k$ 只在 prefill 半开区间（发出 → 首 token）给系统贡献 $r^{\mathrm{prefill}}_k$，只在 decode 半开区间（首 token → 末 chunk）贡献 $r^{\mathrm{decode}}_k$，其它时刻贡献 0。段内不随真实 kernel 快慢变化。

$$
t \in [t^{\mathrm{send}}_k,\ t^{\mathrm{first}}_k)
\quad\text{prefill},\qquad
t \in [t^{\mathrm{first}}_k,\ t^{\mathrm{end}}_k)
\quad\text{decode}
$$
...
也就是说 $t^{\mathrm{first}}$ 是交接：prefill 段在这一时刻结束，decode 段从这一时刻开始。同一时刻若既有「减」又有「加」，实现里先减后加，所以 $t^{\mathrm{first}}$ 这一瞬间不会把两个 $r$ 叠在同一点上；两个半开区间也不重叠。全部事件按时间排序后，相邻两个时刻上瞬时值不变：

$$
R_{\mathrm{prefill}}(t)=\sum_{k \in P(t)} r^{\mathrm{prefill}}_k,\qquad
R_{\mathrm{decode}}(t)=\sum_{k \in D(t)} r^{\mathrm{decode}}_k
$$

其中 $P(t)$ 是此刻仍在 prefill 段的请求，$D(t)$ 是此刻仍在 decode 段的请求。瞬时并发 $C_{\mathrm{prefill}}(t)$ / $C_{\mathrm{decode}}(t)$ 同理，只是每个请求贡献 $1$ 而不是 $r$。$R_{\mathrm{total}}(t)=R_{\mathrm{prefill}}(t)+R_{\mathrm{decode}}(t)$（同一时刻可以有的请求还在等首 token、有的已经在收后续 token）。

例：请求 A 在 $0 \le t \lt 2$ 贡献 $500$ tok/s，请求 B 在 $1 \le t \lt 3$ 贡献 $300$ tok/s，则
...
分析窗一般是整段 benchmark，从 $t_{\min}$ 到 $t_{\max}$，长度 $T$。`effective_*` 的分母是整个 $T$（空窗上 $R=0$ 也算进去）：

$$
\mathrm{effectivePrefill}=\frac{1}{T}\sum_i R_{\mathrm{prefill}}(t_i)\Delta t_i
$$

`active_*` 先用并发当开关：只保留 $C_{\mathrm{phase}}(t) \gt 0$ 的小段，分母改成这些小段时长之和 $T_{\mathrm{active}}$（上例 prefill 的 $T_{\mathrm{active}}=3$，不是整个 $T$）：

$$
\mathrm{activePrefill}=\frac{1}{T_{\mathrm{active}}}\sum_{C_{\mathrm{prefill}}(t_i)\gt 0} R_{\mathrm{prefill}}(t_i)\Delta t_i
$$

上例：`active_prefill` $=(500\cdot 1+800\cdot 1+300\cdot 1)/3=533.3$；若 $T=6$，`effective_prefill` $=1600/6\approx 266.7$。decode 只占墙钟 30% 时，`effective_decode` 大约是 `active_decode` 的 0.3 倍。p50/p90 也是按这些小段的**时长**当权重，不是按请求条数。
...
* `active_prefill_*` 用发出到首 token 的整段 TTFT 当成 prefill。排队、网络、PD 的 KV 传输都在这个窗口里，和 GPU prefill 差得最远。
* `active_decode_*` 用首 token 到末 chunk，把 $(\mathrm{OSL}-1)$ 个后续 token **匀速**摊开。这更接近 serving 的生成阶段，但仍然是客户端墙钟：含后续 token 的网络/SSE、调度间隔、batch 变化、preemption；ATOM 若把多个 token 打进一个 chunk，窗口按 chunk 到达而不是逐 token kernel。GPU 上 Decode 的第一次 forward 往往在客户端收到 T0 **之前**就开了（T0 还在路上），最后一次 kernel 结束也早于客户端收到最后一个 chunk。要 kernel 时间线只能看 engine trace / HIP event。
...
* 整条 PD 系统：$T_{\mathrm{system}}\approx\min(T_P,T_D,T_{\mathrm{KV}})$。P 是 input tok/s、D 是 output tok/s，要用「每秒能养多少条请求」再比，不能把两边的 tok/s 相加。
...
```mermaid
flowchart TD
  A["按时间排列的 forward batch<br/>形状得到本次 token 数 n_i"]
  ...
  H["时间加权平均，同 active 指标<br/>T = sum(R_i * t_fwd_i) / sum(t_fwd_i)"]
